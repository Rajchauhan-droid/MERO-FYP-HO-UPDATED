...
def home_view(request):
    ...
    total_companies = Job.objects.count()
        ...
        if form.is_valid():
            ...
            try:
                email = EmailMessage(contact_name,
                                    content,
                                    contact_email,
                                    ['rijanchauhan03@gmail.com'], #change to your email
...
def terms(request):
    return render(request,'jobapp/terms.html',{})

def privacy(request):
    return render(request,'jobapp/privacy.html',{})